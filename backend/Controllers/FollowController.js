...
const followUser = async (req, res) => {
  ...
  const followAlumni =  async (req, res) => {
    try {
      ...
      if (!userId) {
        return res.status(401).json({ error: 'User must be logged in to follow' });
      }
      ...
      res.status(200).json({ message: 'Successfully followed the alumni', followCount: alumni.followers?.length || 0 });
    } catch (error) {
  ...
  const unfollowAlumni = async (req, res) => {
    try {
      ...
      if (!userId) {
        return res.status(401).json({ error: 'User must be logged in to unfollow' });
      }
      ...
      res.status(200).json({ message: 'Successfully unfollowed the alumni', followCount: alumni.followers?.length || 0 });
    } catch (error) {
